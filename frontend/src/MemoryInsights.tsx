...
export const MemoryInsights: React.FC<MemoryInsightsProps> = ({ userId, apiUrl }) => {
  const [summary, setSummary] = useState<any>(null);
  const [graph, setGraph] = useState<{ nodes: MemoryNode[], edges: MemoryEdge[] } | null>(null);
  ...
  const loadData = async () => {
    ...
    try {
      // 記憶の要約取得
      const summaryResponse = await fetch(`${apiUrl}/api/memories/${userId}/summary?days=30`);
      ...
      // 記憶グラフ取得
      const graphResponse = await fetch(`${apiUrl}/api/memories/${userId}/graph`);
      if (graphResponse.ok) {
    ...
    <div style={{
      ...
      {isExpanded && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div>
              ...
              <div style={{
                ...
                <div style={{ fontSize: '13px', color: '#059669', marginBottom: '8px' }}>
                  記憶総数: <strong>{summary.total_memories}件</strong> （{summary.period_days}日間）
                </div>
              </div>
            ...
            <div>
              ...
              <div style={{
                ...
                <div style={{ fontSize: '13px', color: '#475569', marginBottom: '12px' }}>
                  ノード数: {graph.nodes.length} | 関連性: {graph.edges.length}件
                </div>
                    ...
                    return (
                      <div key={node.id} style={{
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '4px' }}>
                          ...
                          <div style={{ fontSize: '10px', color: '#9ca3af', whiteSpace: 'nowrap', marginLeft: '8px' }}>
                        ...
                        {relatedEdges.length > 0 && (
                          <div style={{ fontSize: '11px', color: '#6b7280', marginTop: '4px' }}>
                            ...
                            {relatedEdges.slice(0, 2).map((edge, idx) => (
                              <span key={idx} style={{ marginLeft: '4px', fontSize: '10px', color: '#9ca3af' }}>
                                ({edge.reason})
                              </span>
                            ))}
                          </div>
          ...
          {/* データなし表示 */}
          {!loading && (!summary || !summary.has_data) && (!graph || graph.nodes.length === 0) && (
            <div style={{
              padding: '24px',
              textAlign: 'center',
              color: '#6b7280',
