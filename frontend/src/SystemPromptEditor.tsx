...
export const SystemPromptEditor: React.FC<SystemPromptEditorProps> = ({ userId, apiUrl }) => {
  ...
  const loadPrompts = async () => {
    try {
      ...
      // デフォルトプロンプトを取得
      const defaultResponse = await fetch(`${apiUrl}/api/system-prompt/default`);
      const defaultData = await defaultResponse.json();
      setDefaultPrompt(defaultData.default_prompt);

      // ユーザープロファイルからカスタムプロンプトを取得
      const profileResponse = await fetch(`${apiUrl}/api/extended-profile/${userId}`);
      const profileData = await profileResponse.json();

      if (profileData.profile.profile_settings.custom_system_prompt) {
        setCustomPrompt(profileData.profile.profile_settings.custom_system_prompt);
        setUseCustom(true);
      } else {
  ...
  const handleSave = async () => {
    try {
      ...
      // カスタムプロンプトを更新
      profileData.profile.profile_settings.custom_system_prompt = useCustom ? customPrompt : null;
    ...
    <div style={{
      ...
      <div style={{ marginBottom: '16px' }}>
        ...
        <div style={{ marginBottom: '12px' }}>
          <div style={{
            ...
            💡 使用可能な変数: {'{ai_name}'}, {'{ai_personality}'}, {'{user_context}'}, {'{conversation_context}'}, {'{response_pattern}'}
          </div>
        </div>
      ...
      <div style={{
        ...
        <ul style={{ marginTop: '8px', paddingLeft: '20px' }}>
          <li>カスタムプロンプトを使用すると、AIの性格や応答スタイルを自由にカスタマイズできます</li>
